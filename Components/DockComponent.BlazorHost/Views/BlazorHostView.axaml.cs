...
public partial class BlazorHostView : UserControl, IViewFor<BlazorHostViewModel>
{
    public BlazorHostView()
    {
        System.Diagnostics.Debug.WriteLine(
            "🏗️ BlazorHostView constructor START");
        ...
        System.Diagnostics.Debug.WriteLine(
            "🏗️ About to call InitializeComponent");
        InitializeComponent();
        ...
        this.WhenAnyValue(x => x.ViewModel)
            ...
            {
                System.Diagnostics.Debug.WriteLine(
                    $"🔗 ViewModel changed to: {vm?.GetType().Name ?? "NULL"}");
                SetupViewModelSubscriptions(vm);
            });

        System.Diagnostics.Debug.WriteLine(
            "🏗️ BlazorHostView constructor END");
    }
    ...
    private void SetupViewModelSubscriptions(BlazorHostViewModel? viewModel)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine(
                "⚠️ SetupViewModelSubscriptions: viewModel is null");
            return;
        }

        System.Diagnostics.Debug.WriteLine(
            "🔄 Setting up ViewModel subscriptions");
        ...
        viewModel.WhenAnyValue(x => x.CurrentUrl)
            ...
            {
                System.Diagnostics.Debug.WriteLine(
                    $"🔄 View: CurrentUrl changed to: '{url}'");
                ...
                if (!string.IsNullOrEmpty(url) && BlazorWebView != null)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine(
                            $"🔧 Forcing WebView.Address = '{url}'");
                        BlazorWebView.Address = url;
                        System.Diagnostics.Debug.WriteLine(
                            $"✅ WebView.Address set successfully");
                    }
                    catch (Exception ex)
                    {
                        System.Diagnostics.Debug.WriteLine(
                            $"❌ Error setting WebView.Address: {ex.Message}");
                    }
                }
            });
        ...
        viewModel.WhenAnyValue(x => x.IsLoaded)
            ...
            {
                System.Diagnostics.Debug.WriteLine(
                    $"🔄 View: IsLoaded changed to: {isLoaded}");
            });
        ...
        viewModel.WhenAnyValue(x => x.StatusMessage)
            ...
            {
                System.Diagnostics.Debug.WriteLine(
                    $"🔄 View: StatusMessage changed to: '{status}'");
            });

        System.Diagnostics.Debug.WriteLine(
            "✅ All ViewModel subscriptions set up");
    }

    public static readonly StyledProperty<BlazorHostViewModel?>
        ViewModelProperty =
            AvaloniaProperty.Register<BlazorHostView, BlazorHostViewModel?>(
                nameof(ViewModel));
