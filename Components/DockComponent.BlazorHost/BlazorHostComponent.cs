...
public class BlazorHostComponent : IDockComponent, IDisposable
{
    ...
    {
        // Register services
        var webHostManager = context.Services
            .FirstOrDefault(e => e.ServiceType == typeof(WebHostManager))
        ...
        if (webHostManager == null)
        {
            ...
            var stylesUri =
                new Uri("avares://DockComponent.BlazorHost/Styles.axaml");
            ...
            var sharedThemeService =
                context.Services
                    .FirstOrDefault(s => s.ServiceType == typeof(IThemeService))
                    ?.ImplementationInstance as IThemeService;
            var sharedDashboardService =
                context.Services
                    .FirstOrDefault(s =>
                        s.ServiceType == typeof(IDashboardService))
                    ?.ImplementationInstance as IDashboardService;

            Console.WriteLine(
                $"🔗 [BLAZOR HOST] Found shared services - Theme: {sharedThemeService != null}, Dashboard: {sharedDashboardService != null}");

            // Create direct FluentBlazor dashboard document
            var fluentDashboard =
                new FluentDashboardViewModel(sharedThemeService,
            ...
            var embeddedDashboard =
                new EmbeddedDashboardViewModel(sharedThemeService,
                    sharedDashboardService);

            // Register documents/tools
            context.RegisterDocument("FluentDashboard",
                fluentDashboard); // Primary dashboard
            context.RegisterDocument("BlazorHost",
                blazorHost); // Browser simulator
            context.RegisterDocument("WebViewTest", new WebViewTestViewModel());
            context.RegisterDocument("EmbeddedDashboard",
                embeddedDashboard); // Web service

            // Wire up the embedded dashboard to automatically load in BlazorHost when ready
            embeddedDashboard.PropertyChanged += (sender, e) =>
            {
                if (e.PropertyName == nameof(embeddedDashboard.CurrentUrl) &&
                    !string.IsNullOrEmpty(embeddedDashboard.CurrentUrl))
                {
                    var dashboardUrl =
                        $"{embeddedDashboard.CurrentUrl}/dashboard-embedded";
                    Console.WriteLine(
                        $"🔗 [BLAZOR HOST] Auto-loading embedded dashboard: {dashboardUrl}");
                    blazorHost.LoadUrl(dashboardUrl);
                }
            };

            Console.WriteLine(
                "✅ [BLAZOR HOST] Registered documents: FluentDashboard, BlazorHost, WebViewTest, EmbeddedDashboard");
        }
    }
    ...
    private void StartBlazorServer(IDockComponentContext context)
    {
        try
        {
            ...
            var loggerFactory =
                context.Services
                    ...
                    ?.ImplementationInstance as ILoggerFactory;
            var logger = loggerFactory?.CreateLogger<BlazorServerHost>() ??
                         new NullLogger<BlazorServerHost>();
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    Console.WriteLine(
                        "🚀 [BLAZOR HOST] Plugin started its own Blazor server!");
                }
                catch (Exception ex)
                {
                    logger.LogError(ex,
                        "Failed to start Blazor server from plugin");
                }
            });
        }
        catch (Exception ex)
        {
            Console.WriteLine(
                $"❌ [BLAZOR HOST] Failed to start Blazor server: {ex.Message}");
        }
    }
    ...
    public void Dispose()
    {
            ...
            if (_blazorServer != null)
            {
                _ = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        Console.WriteLine(
                            "🛑 [BLAZOR HOST] Plugin stopped its Blazor server");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(
                            $"❌ [BLAZOR HOST] Error stopping Blazor server: {ex.Message}");
                    }
                });
            }
        }
        ...
        {
            Console.WriteLine(
                $"❌ [BLAZOR HOST] Error disposing component: {ex.Message}");
        }
    }
}
