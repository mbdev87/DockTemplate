...
public static class SolutionExplorerMessages
{
    ...
    public const string FILE_SELECTED =
        "SolutionExplorerComponent_FileSelected";

    public const string DIRECTORY_EXPANDED =
        "SolutionExplorerComponent_DirectoryExpanded";

    public const string DIRECTORY_COLLAPSED =
        "SolutionExplorerComponent_DirectoryCollapsed";
...
public record FileSelectedData(
    string FilePath,
    string FileName,
    string FileExtension,
    DateTime Timestamp);

public record DirectoryExpandedData(
...
public static class SolutionExplorerMessageHelper
{
    public static void EmitFileSelected(string filePath)
    {
        ...
        var data = new FileSelectedData(filePath, fileInfo.Name,
            fileInfo.Extension, DateTime.UtcNow);
        var json = JsonSerializer.Serialize(data);
        var message =
            new ComponentMessage(SolutionExplorerMessages.FILE_SELECTED, json);
        MessageBus.Current.SendMessage(message);
    }

    public static void EmitDirectoryExpanded(string directoryPath,
        int itemCount)
    {
        var data =
            new DirectoryExpandedData(directoryPath, itemCount,
                DateTime.UtcNow);
        var json = JsonSerializer.Serialize(data);
        var message =
            new ComponentMessage(SolutionExplorerMessages.DIRECTORY_EXPANDED,
                json);
        MessageBus.Current.SendMessage(message);
    }
    ...
    public static void EmitDirectoryCollapsed(string directoryPath)
    {
        ...
        var json = JsonSerializer.Serialize(data);
        var message =
            new ComponentMessage(SolutionExplorerMessages.DIRECTORY_COLLAPSED,
                json);
        MessageBus.Current.SendMessage(message);
    }
}