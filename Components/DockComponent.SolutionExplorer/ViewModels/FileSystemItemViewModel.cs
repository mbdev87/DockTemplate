...
public class FileSystemItemViewModel : ReactiveObject
{
    ...
    [Reactive]
    public ObservableCollection<FileSystemItemViewModel>
    ...
    private void LoadChildren()
    {
        ...
        {
            LogError($"❌ Error loading children for {FullPath}: {ex.Message}",
                FullPath);
            SendErrorEntry("Error",
                $"Failed to load directory contents: {ex.Message}", FullPath);
        }
    }
    ...
    private void OpenFile()
    {
        ...
        {
            LogWarn(
                $"🖼️ Image file detected: {Name} - TODO: Open with image viewer",
                FullPath);
        ...
        else
        {
            ...
            LogError($"🔒 Binary/unsupported file type: {extension} ({Name})",
                FullPath);

            // Send as error entry but don't make it navigable since it's a binary file
            SendErrorEntry("Warning",
                $"Cannot open binary file: {extension} files are not supported",
                null);
        }
    }
    ...
    private bool IsTextFileByContent(string filePath)
    {
        try
        {
            ...
            {
                byte b = buffer[i];
                if ((b >= 32 && b <= 126) || b == 9 || b == 10 ||
                    b == 13) // Printable ASCII + tab/newline
                    printableCount++;
            }
    ...
    private void SendErrorEntry(string level, string message,
        string? filePath = null, int? lineNumber = null)
    {
        ...
        var componentMessage =
            new ComponentMessage("SolutionExplorer_ErrorReported", json);
        MessageBus.Current.SendMessage(componentMessage);
    }
    ...
    private void CheckForCommonIssues(string filePath)
    {
        try
        {
            ...
            {
                SendErrorEntry("Warning",
                    $"Large file detected ({fileInfo.Length / 1024 / 1024}MB) - may impact performance",
                    filePath, 1);
            }
            ...
            if (extension == ".cs" && !fileName.EndsWith(".g.cs"))
            {
                ...
                var projectFileInDir =
                    Path.Combine(Path.GetDirectoryName(filePath) ?? "",
                        "*.csproj");
                if (!File.Exists(projectFile) && Directory
                        .GetFiles(Path.GetDirectoryName(filePath) ?? "",
                            "*.csproj").Length == 0)
                {
                    SendErrorEntry("Warning",
                        "C# file found but no project file (.csproj) in directory",
                        filePath, 1);
                }
            }
            ...
            {
                SendErrorEntry("Warning",
                    "C# file name contains spaces - consider using PascalCase",
                    filePath, 1);
            }
        ...
        catch (Exception ex)
        {
            ...
            LogInfo(
                $"Unable to check file issues for {filePath}: {ex.Message}");
        }
    }
    ...
    private void CreateRealisticCSharpErrors(string filePath)
    {
        try
        {
                 ...
                 i++) // Check first 50 lines only
            {
                ...
                {
                    SendErrorEntry("Warning",
                        $"TODO comment found: {line.Trim()}", filePath,
                        lineNumber);
                }
                ...
                {
                    SendErrorEntry("Warning",
                        "Debug Console.WriteLine should be removed in production",
                        filePath, lineNumber);
                }
                ...
                {
                    SendErrorEntry("Error",
                        "Empty catch block - exceptions silently ignored",
                        filePath, lineNumber);
                }

                if (line.Contains("public class") &&
                    !filePath.Contains(line.Split(' ')[2]))
                {
                    var className = line.Split(' ')
                        .Where(s => !string.IsNullOrEmpty(s)).Skip(2)
                        .FirstOrDefault();
                    if (!string.IsNullOrEmpty(className) &&
                        !Path.GetFileName(filePath).StartsWith(className))
                    {
                        SendErrorEntry("Warning",
                            $"Class name '{className}' should match filename",
                            filePath, lineNumber);
                    }
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            SendErrorEntry("Info",
                "Code analysis: File contains potential issues", filePath, 10);
        }
    }
    ...
    private void CreateRealisticXamlErrors(string filePath)
    {
        try
        {
            ...
            for (int i = 0; i < Math.Min(lines.Length, 30); i++)
            {
                ...
                {
                    SendErrorEntry("Warning", "Binding without Path property",
                        filePath, lineNumber);
                }
                ...
                if (line.Contains("x:Name=\"") &&
                    line.Count(c => c == '"') % 2 != 0)
                {
                    SendErrorEntry("Error", "Unclosed x:Name attribute",
                        filePath, lineNumber);
                }
            }
        }
        ...
        {
            SendErrorEntry("Info",
                "XAML validation: Potential binding issues detected", filePath,
                5);
        }
    }
}