    ...
    public class LoggingDataService : ReactiveObject
    {
        ...
        private void ConnectToSharedLogging()
        {
            try
            {
                ...
                SharedLoggingService.Instance.LogEntries.CollectionChanged +=
                    (sender, e) =>
                    {
                ...
                Console.WriteLine(
                    "[OutputComponent] Connected to SharedLoggingService - showing centralized logs");
            }
            catch (Exception ex)
            {
                Console.WriteLine(
                    $"[OutputComponent] Failed to connect to SharedLoggingService: {ex.Message}");
        ...
        private void SetupFallbackMessageListener()
        {
            ...
            var startupEntry = new LogEntry("Info",
                "✅ Output component initialized - listening for log messages",
                "Output", DateTime.Now);
        ...
        private void ApplyFilters()
        {
            ...
            {
                filtered =
                    filtered.Where(entry => entry.Level == SelectedLogLevel);
            }
