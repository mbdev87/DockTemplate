...
public class ThemeService(ISettingsService settingsService) : IThemeService
{
    ...
    public async Task InitializeFromSettingsAsync()
    {
        try
        {
            ...
            var savedThemeIndex = settingsService.Settings.Theme.ThemeIndex;

            Logger.Info(
                $"[ThemeService] Initializing theme from settings: index {savedThemeIndex}");
        ...
        {
            Logger.Error(ex,
                "Failed to initialize theme from settings, using default");
            await Dispatcher.UIThread.InvokeAsync(() =>
        ...
        try
        {
            ...
            if (File.Exists(settingsPath))
            {
                ...
                if (settings != null)
                {
                    ...
                    DockComponent.Base.GlobalSettings.IsDarkTheme =
                        settings.Theme.IsDarkTheme;
                    DockComponent.Base.GlobalSettings.ThemeIndex =
                        settings.Theme.ThemeIndex;
                    DockComponent.Base.GlobalSettings.EnableAcrylic =
                        settings.UI.EnableAcrylic;
                    DockComponent.Base.GlobalSettings.EnableAnimations =
                        settings.UI.EnableAnimations;

                    var savedThemeIndex = settings.Theme.ThemeIndex;
                    Logger.Info(
                        $"[ThemeService] Loading theme SYNC from settings: index {savedThemeIndex}");
                    Logger.Info(
                        $"[ThemeService] Global settings populated - Acrylic: {DockComponent.Base.GlobalSettings.EnableAcrylic}");
                    ApplyThemeWithoutSaving(savedThemeIndex);
                    return;
                }
            }
            ...
            Logger.Info(
                "[ThemeService] No settings file found, using default dark theme");
            ApplyThemeWithoutSaving(1); // Default to dark theme
        }
        catch (Exception ex)
        {
    ...
    public void Switch(int index)
    {
        ...
        Logger.Info(
            $"[ThemeService] Switching theme to: {newTheme} (index: {index})");
        ...
        settingsService.SetTheme(newTheme == ThemeVariant.Dark);
