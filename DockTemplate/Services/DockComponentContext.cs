...
public class DockComponentContext : IDockComponentContext
{
    ...
    public DockComponentContext(DockFactory dockFactory,
        IServiceCollection services, Guid componentInstanceId)
    {
    ...
    private void InjectSharedHostServices()
    {
        try
        {
            ...
            if (hostServiceProvider != null)
            {
                ...
                var themeService =
                    hostServiceProvider.GetService<IThemeService>();
                ...
                var loggingService =
                    hostServiceProvider.GetService<LoggingService>();
                ...
                var interPluginLogger =
                    hostServiceProvider.GetService<InterPluginLogger>();
                ...
                Console.WriteLine(
                    $"[DockComponentContext] Injected shared host services into component {_componentInstanceId}");
            }

            // CRITICAL: Always provide SharedLoggingService to ensure consistent logging across components
            Services.AddSingleton<ISharedLoggingService>(provider =>
                SharedLoggingService.Instance);

            // Initialize shared logging for this component
            SharedLoggingService.Instance.ConfigureComponentLogging(
                $"Component-{_componentInstanceId}");
        }
        catch (Exception ex)
        {
            // Don't fail component registration if shared service injection fails
            Console.WriteLine(
                $"[DockComponentContext] Failed to inject shared services: {ex.Message}");
        }
    }
    ...
    public void RegisterResources(Uri resourceUri)
    {
        try
        {
            ...
            Console.WriteLine(
                $"[ComponentContext] Loaded styles from: {resourceUri}");
        }
        catch (Exception ex)
        {
            Console.WriteLine(
                $"[ComponentContext] Failed to load styles from {resourceUri}: {ex.Message}");
        }
    }

    public void RegisterTool(string id, object toolViewModel,
        DockPosition position = DockPosition.Left, bool isPrimary = false)
    {
        var registration = new ComponentRegistration(id, toolViewModel,
            position, _componentInstanceId, isPrimary);
        _registeredTools.Add(registration);

        // CRITICAL: Also store in global ComponentRegistry so DockFactory can find it
        ComponentRegistry.Instance.AddComponents(new[] { registration },
            Array.Empty<ComponentRegistration>());

        var primaryStatus = isPrimary ? " [PRIMARY]" : "";
        Console.WriteLine(
            $"[ComponentContext] Registered tool: {id} -> {toolViewModel.GetType().Name} at {position}{primaryStatus} (Instance: {_componentInstanceId}) -> Added to global registry");
    }

    public void RegisterDocument(string id, object documentViewModel,
        DockPosition position = DockPosition.Document)
    {
        var registration = new ComponentRegistration(id, documentViewModel,
            position, _componentInstanceId);
        _registeredDocuments.Add(registration);

        // CRITICAL: Also store in global ComponentRegistry so DockFactory can find it  
        ComponentRegistry.Instance.AddComponents(
            Array.Empty<ComponentRegistration>(), new[] { registration });

        Console.WriteLine(
            $"[ComponentContext] Registered document: {id} -> {documentViewModel.GetType().Name} at {position} (Instance: {_componentInstanceId}) -> Added to global registry");
    }

    public IReadOnlyList<ComponentRegistration> RegisteredTools =>
        _registeredTools.AsReadOnly();

    public IReadOnlyList<ComponentRegistration> RegisteredDocuments =>
        _registeredDocuments.AsReadOnly();
}