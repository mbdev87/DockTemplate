...
public class PluginInstallationService : BackgroundService
{
    ...
    public PluginInstallationService(IServiceProvider serviceProvider)
    {
        ...
        MessageBus.Current.Listen<InstallPluginMessage>()
            .Subscribe(async message =>
                await HandleInstallPluginRequest(message));
    ...
    private async Task HandleInstallPluginRequest(InstallPluginMessage message)
    {
        ...
        {
            Logger.Info(
                $"Starting plugin installation: {message.PluginFileName}");
            MessageBus.Current.SendMessage(
                new PluginInstallationStartedMessage(message.PluginFileName));
            await Task.Delay(500);
            await InstallPluginAsync(message.PluginPath);
            await Task.Delay(500);
            MessageBus.Current.SendMessage(
                new PluginInstallationCompletedMessage(message.PluginFileName,
                    true));
            Logger.Info(
        ...
        {
            Logger.Error(ex,
                $"Failed to install plugin: {message.PluginFileName}");
            MessageBus.Current.SendMessage(
                new PluginInstallationCompletedMessage(
    ...
    private async Task InstallPluginAsync(string pluginPath)
    {
        ...
        var localAppDataPath =
            PluginDirectoryService.GetLocalAppDataPluginPath();
        ...
        {
            Logger.Info(
                $"Plugin {pluginFileName} already installed - will overwrite and reload");
        }
        ...
        using (var archive = ZipFile.OpenRead(copiedPluginPath))
        {
            foreach (var entry in archive.Entries)
            {
                ...
                var destinationPath =
                    Path.Combine(componentsPath, entry.FullName);
        ...
        if (extractedFiles == 0)
        {
            throw new InvalidOperationException(
                $"Plugin {pluginFileName} is empty or contains no files to extract");
        }
    ...
    private async Task HotLoadNewPluginAsync(string componentsPath)
    {
        try
        {
            ...
            var componentContext =
                _serviceProvider.GetService<DockComponentContext>();

            // Detailed diagnostic logging for service availability
            Logger.Info($"Service availability check:");
            Logger.Info(
                $"  ComponentLoader: {(componentLoader != null ? "✅ Available" : "❌ MISSING")}");
            Logger.Info(
                $"  DockFactory: {(dockFactory != null ? "✅ Available" : "❌ MISSING")}");
            Logger.Info(
                $"  DockComponentContext: {(componentContext != null ? "✅ Available" : "❌ MISSING")}");

            if (componentLoader == null)
            {
                Logger.Error(
                    "❌ ComponentLoader service is not registered in DI container - cannot hot-load plugins");
                return;
            }
            ...
            {
                Logger.Error(
                    "❌ DockFactory service is not registered in DI container - cannot hot-load plugins");
                return;
            }
            ...
            {
                Logger.Error(
                    "⚠️ DockComponentContext service is not registered - this is expected with new architecture (components create their own contexts)");
            ...
            Logger.Info(
                $"  Total loaded components: {registry.LoadedComponents.Count}");
            Logger.Info($"  Registered tools: {registry.ComponentTools.Count}");
            Logger.Info(
                $"  Registered documents: {registry.ComponentDocuments.Count}");
            ...
            Logger.Info(
                "✅ Hot-loading completed! New plugin should now be visible in UI.");
        }
        catch (Exception ex)
        {
